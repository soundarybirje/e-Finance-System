...
<thead>
  ...
  <?php
	...
	$totins=0;
	$tot=0;
	$sql="select * from  loan_allot a,loan_details b,customer_details c where a.loan_id=b.loan_id and a.customer_id=c.customer_id and c.customer_id='$customer_id'";
	$res=mysqli_query($conn,$sql);
...
<thead>
  ...
  <?php
	...
	$sn=1;
	$totpa=0;
	$sql="select * from pigmy_collect a,customer_details b,pigmy_agent pa where a.customer_id=b.customer_id and a.pigmy_agent_id=pa.pigmy_agent_id and b.customer_id='$customer_id'";
	$res=mysqli_query($conn,$sql);
	while($row=mysqli_fetch_array($res))
	{
